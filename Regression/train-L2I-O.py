...
def train(labeled_trainloader, unlabeled_trainloader, train_val_loader, model, target_model, tmp_model, optimizer, ema_optimizer, target_optimizer, criterion, epoch, use_cuda, rampup_length):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        Lu_tmp = ((torch.tanh(logits_u) - torch.tanh(targets_u)) ** 2).mean()
        ...
        tmp_lr = 10 * optimizer.param_groups[0]['lr']
        ...
        p_grad = torch.autograd.grad(train_val_loss, p, retain_graph=True)[0]
        meta_grads = torch.autograd.grad(p, model.parameters(), p_grad)
